...
namespace neuro {
  ...
  Population::Population(size_t size, const std::vector<int>& structure) {
    for (size_t i = 0; i < size; i++) {
      individuals.emplace_back(std::make_shared<Individual>(structure, maker::activationIdentity()));
    }
  }
